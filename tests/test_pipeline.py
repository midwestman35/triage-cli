...
def test_triage_one_on_phase_fires_lm_only_when_no_logs(
    ...
    async def fake_triage(  # noqa: ARG001
        _bundle, *, model=None, verbose=False, redact_enabled=True,
    ):
        return (
            LLMTriageOutput(finding="x", confidence="low", evidence=[], suggested_note="y"),
            RedactionCounts(enabled=True),
        )
...
def test_triage_one_on_phase_fires_all_steps_with_logs(
    ...
    async def fake_anchor(  # noqa: ARG001
        _ticket, model=None, *, redact_enabled=True, verbose=False,
    ):
        return None
