...
- **Anthropic HTTP SDK as the default Claude provider.** Rejected (already prohibited in `CLAUDE.md`). The OAuth-only enterprise seat does not support a provisioned API key; the `claude --print` subprocess provider was the workaround until both the `claude` and `openai` providers were removed 2026-05-14 in favor of `unleash` + `codex` only (see `docs/adr/0002-prune-claude-openai-providers.md`). If a future Anthropic API path is reintroduced, the OAuth-only constraint still applies.
