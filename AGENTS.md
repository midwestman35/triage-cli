...
This file provides guidance to Codex (codex.com) and other AGENTS.md-compatible agents when working with code in this repository.

> **Kept in sync with `CLAUDE.md`.** Edit both together — they have the same content and exist only because Claude Code and Codex / other agents each look for a different filename.
